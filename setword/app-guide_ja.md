...
- **ホーム**：現在のプロジェクトと次の操作を表示し、準備中、進行中、完了、放棄などの状態別に整理します。
  「クイックスタート」は Ravelry、編み物ノート、棒針、かぎ針、毛糸、作品、編み方、ヘルプに分かれ、項目の列を左右にスワイプできます。「作品」からはファイル、項目順、作品パッケージの読込、新規作品、Ravelry 読込を直接開けます。「ヘルプ」にはアプリ機能、棒針・かぎ針図案、編み物ノート、カウンター、スワッチシミュレーターの説明とご意見への入口があります。他の項目は従来どおり下の角丸グループに入口が開き、上の矢印が選択中の項目を示します。もう一度選ぶか「折りたたむ」で閉じられます。
...
- 「仕様分析」は手編み糸の包装重量と長さから 100g 当たりの長さ、線密度、Nm、残り長さを、コーン糸の Nm と本数から線密度と長さを換算します。太さ分類から一般的な棒針・かぎ針・メリヤスゲージ範囲も表示します。繊維密度による値は「等価中実直径」と明記し、かさ高性・撚り・空気を含む実際の糸径ではありません。スワッチシミュレーターは独立した編み物ツールです。
- 毛糸をプロジェクトに関連付け、どちらのカテゴリーも使用量をグラムで記録して費用を見積もります。手編み糸の重量と長さが揃っている場合は、使用した長さも重量比で推定します。
